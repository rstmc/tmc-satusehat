...
class ImunisasiDasar extends ImmunizationBase
{
    public function push($row, $encounterId)
    {
        ...
        $payload = [
            ...
            "vaccineCode" => [
                "coding" => [
                    [
                        ...
                        "code" => $row['VaccineCodeKFA_1'] ?? 'VG17',
                        "display" => $row['VaccineDisplayKFA_1'] ?? 'HIB'
                    ],
                    [
                        "system" => "http://sys-ids.kemkes.go.id/kfa",
                        "code" => $row['VaccineCodeKFA_2'] ?? 'VG45',
                        "display" => $row['VaccineDisplayKFA_2'] ?? 'HepB'
                    ],
                    [
                        "system" => "http://sys-ids.kemkes.go.id/kfa",
                        "code" => $row['VaccineCodeKFA_3'] ?? 'VG107',
                        "display" => $row['VaccineDisplayKFA_3'] ?? 'DTAP'
                    ],
                    [
                        "system" => "http://hl7.org/fhir/sid/cvx",
                        "code" => $row['VaccineCodeCVX'] ?? '102',
                        "display" => $row['VaccineDisplayCVX'] ?? 'DTP-Hib-Hep B'
                    ]
                ]
            ],
            "patient" => [
            ...
            "protocolApplied" => [
                [
                    "doseNumberPositiveInt" => isset($row['DoseNumber']) ? (int)$row['DoseNumber'] : 1
                ]
            ]
        ];
