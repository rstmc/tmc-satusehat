...
class ImunisasiDilakukanOlehNakesDilaporkanKIPIOlehPasein extends ImmunizationBase
{
    public function push($row, $encounterId)
    {
        ...
        $reactionDate = isset($row['ReactionDate']) ? date('Y-m-d', strtotime($row['ReactionDate'])) : date('Y-m-d');
        ...
        $payload = [
            ...
            "vaccineCode" => [
                "coding" => [
                    ...
                    [
                        "system" => "http://sys-ids.kemkes.go.id/kfa",
                        "code" => $row['KfaCodeHIB'] ?? 'VG17',
                        "display" => $row['KfaDisplayHIB'] ?? 'HIB'
                    ],
                    [
                        "system" => "http://sys-ids.kemkes.go.id/kfa",
                        "code" => $row['KfaCodeHepB'] ?? 'VG45',
                        "display" => $row['KfaDisplayHepB'] ?? 'HepB'
                    ],
                    [
                        "system" => "http://sys-ids.kemkes.go.id/kfa",
                        "code" => $row['KfaCodeDTAP'] ?? 'VG107',
                        "display" => $row['KfaDisplayDTAP'] ?? 'DTAP'
                    ],
                    [
